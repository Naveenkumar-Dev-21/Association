...
import OuterCollegeModal from '../components/OuterCollegeModal';
...
const CreateEvent = () => {
  ...
  // Set default organizing department from admin profile
  useEffect(() => {
    if (admin && admin.cellsAndAssociation && !formData.cellsAndAssociation) {
      const defaultDept = admin.cellsAndAssociation === 'OT' ? 'IT' : admin.cellsAndAssociation;
      setFormData(prev => ({
        ...prev,
        cellsAndAssociation: defaultDept,
        organizingBody: defaultDept
      }));
    }
  }, [admin]);
  ...
  const handleOuterCollegeSave = async (outerCollegeData) => {
    setIsLoading(true);
    try {
      ...
      formDataToSend.append('eventType', JSON.stringify(['Inter College']));
      formDataToSend.append('mode', 'Offline');
      formDataToSend.append('eventDate', new Date().toISOString());
      formDataToSend.append('eventCoordinator[name]', 'TBD');
      formDataToSend.append('eventCoordinator[contact]', '0000000000');
      formDataToSend.append('cellsAndAssociation', formData.cellsAndAssociation || 'IT');
      formDataToSend.append('description', 'See poster for details');
      formDataToSend.append('rules', 'See poster for details');
      formDataToSend.append('registrationLink', '#');
      formDataToSend.append('maxParticipants', '100');
      ...
      console.error('Error creating outer college event:', error);
      toast.error(error.response?.data?.message || 'Failed to create event');
    } finally {
      setIsLoading(false);
      setShowOuterCollegeModal(false);
    }
  };

  const handleOuterCollegeCancel = () => {
    setShowOuterCollegeModal(false);
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'organizingBody') {
      setFormData(prev => ({
        ...prev,
        organizingBody: value,
        cellsAndAssociation: value
      }));
      ...
      case 1:
        ...
        if (registrationType === 'outer') {
          if (!formData.outerCollegeData) {
            toast.error('Please fill in outer college event details');
            return false;
          }
        }
  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 p-8">
        ...
        {/* Outer College Modal */}
        <OuterCollegeModal
          isOpen={showOuterCollegeModal}
          onClose={handleOuterCollegeCancel}
          onSave={handleOuterCollegeSave}
        />

        {/* Form Content - Only show for Platform registration type */}
        {!showRegistrationModal && !showOuterCollegeModal && registrationType === 'platform' && (
          <>
            {/* Step Indicator */}
            {renderStepIndicator()}
